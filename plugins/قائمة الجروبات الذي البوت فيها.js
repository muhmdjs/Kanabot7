const handler = async (m, { conn, usedPrefix, args }) => {
  ...
  if (args.length === 0) {
    ...
    conn.reply(m.chat, `❗ تنسيق أمر غير صحيح. يستخدم "${usedPrefix}قائمة_الجروبات" untuk daftar grup atau "${usedPrefix}groups [nomor_urutan]" untuk informasi grup tertentu.`, m);
  }
};

handler.menu = ['grouplist'];
handler.tags = ['tools'];
handler.command = /^(قائمة_الجروبات)$/i;
