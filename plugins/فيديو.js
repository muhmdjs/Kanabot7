  ...
  try {
    ...
    if (command == 'فيديو') {
      ...
      try {
        ...
        for (let i = 0; i < 3; i++) {
          const videoUrl = mediaa.results[i]?.video;
          if (videoUrl) {
            await conn.sendMessage(m.chat, {
              video: { url: videoUrl },
              fileName: `video${i + 1}.mp4`,
              caption: `*『b o t k a n a』*\n *انا غير مسؤل عن ذنوب اغانيك*`,
...
async function ytMp4(url) {
  return new Promise(async (resolve, reject) => {
    ytdl.getInfo(url).then(async (getUrl) => {
      ...
      const title = getUrl.videoDetails.title;
      const thumb = getUrl.player_response.microformat.playerMicroformatRenderer.thumbnail.thumbnails[0].url;
      resolve({ title, result: tinyUrl, results: resultFix, thumb });
    }).catch(reject);
  });
}
...
async function ytPlayVid(query) {
  return new Promise((resolve, reject) => {
    yts(query).then(async (getData) => {
      ...
      const getVideo = await ytMp4(url[0]);
      resolve({ results: url.slice(0, 3), ...getVideo });
    }).catch(reject);
  });
                                                                                                                                }
